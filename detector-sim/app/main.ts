...
export class Main {
  ...
  constructor(http:Http, fb:FormBuilder) {
    this.http = http;
    this.serverAddress = getFromLocalStorage('serverAddress', 'https://1sz2z4bt93.execute-api.eu-central-1.amazonaws.com/cchehu/');
  ...
  passage(sensorId:string) {
    ...
    this.http.post(this.serverAddress + 'gate-passed?sensorID=' + sensorId + '&timestamp=' + time)
      .subscribe(
        response => {
